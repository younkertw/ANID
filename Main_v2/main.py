...
class ANID(object):
	...
	def output_layer(self, outputs, num_classes):
		...
		final_output = tf.compat.v1.layers.dense(
				inputs= outputs,
				units= num_classes,
...
def main(unused_args):
	...
	Input_X = tf.compat.v1.placeholder( tf.float32, [None, FLAGS.seq_len, FLAGS.num_features])
	Input_Y = tf.compat.v1.placeholder( tf.float32, [None, FLAGS.seq_len, num_classes])
	...
	gpu_options = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=0.30)
	...
	if FLAGS.train:	
		...
		with tf.compat.v1.Session(config=config) as sess:
			sess.run(tf.compat.v1.global_variables_initializer()) # Run the initializer
	...
	if FLAGS.test:
		...
		with tf.compat.v1.Session(config= config) as sess:
			...
			#print (tf.trainable_variables())
			get_total_para(tf.compat.v1.trainable_variables())
