...
def attention(query, key, value):
	...
	output = tf.matmul(query, key, transpose_b=True) / (tf.cast(tf.shape(input=query)[2], tf.float32) ** 0.5)
	...
	# 	Layer normalization
	layer = tf.keras.layers.LayerNormalization(axis=2) #Eric
	output = layer(output) #Eric
...
def pos_encoding(input_x, num_hidden, seq_len, keep_prob):
	...
	embedded_input = tf.compat.v1.layers.dense(
				inputs = input_x,
				units = num_hidden,
				activation = None,
				name = 'input_dense')
...
def ffn(encoding, num_hidden):
	...
	encoding += tf.compat.v1.layers.dense(
			inputs=dense,
			units= num_hidden,
			activation=None,
			name="ffn_add",
...
def sda(encoding, num_hidden, initial_input, seq_len):
	...
	tf.tile(decoder_input, multiples=tf.concat(([tf.shape(input=initial_input)[0]], [1], [1]), axis=0)),
	encoding,
	encoding,
	)
