...
def weighted_crossentropy(y_true, y_pred):
	...
	output = y_pred
	output /= tf.reduce_sum(input_tensor=y_pred, axis=-1, keepdims=True)
	output = tf.clip_by_value(output, 1e-5, 1. - 1e-5)
	pos_weight = tf.cast(tf.reduce_sum(input_tensor=y_true[:, :, 0]), tf.float32) / (tf.reduce_sum(input_tensor=y_true[:, :, 1]) +1e-5)
	loss = y_true[:,:,1]*pos_weight*tf.math.log(output)[:,:,1] + y_true[:,:,0]*1*tf.math.log(output)[:,:,0]
	xent = - tf.reduce_mean(input_tensor=loss)
	return xent
...
def get_result(test_pred, testY, threshold):
	...
	_test_precision,_test_recall,_test_f1,_support = precision_recall_fscore_support(targ, pred, zero_division=0) #Eric
